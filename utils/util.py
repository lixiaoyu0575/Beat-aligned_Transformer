...
def plot_metric_all(metric_train, metric_val, metric_test, metric, save_path):
    textsize = 15
    marker = 5
    plt.figure()
    fig, ax1 = plt.subplots()
    ax1.plot(metric_train, 'r')
    ax1.plot(metric_val, 'b')
    ax1.plot(metric_test, 'g')
    ax1.set_ylabel(metric)
    plt.xlabel('epoch')
    plt.grid(b=True, which='major', color='k', linestyle='-')
    plt.grid(b=True, which='minor', color='k', linestyle='--')
    lgd = plt.legend(['train', 'val', 'test'], markerscale=marker, prop={'size': textsize, 'weight': 'normal'})
    ax = plt.gca()
    plt.title(metric)
    for item in ([ax.title, ax.xaxis.label, ax.yaxis.label] +
                 ax.get_xticklabels() + ax.get_yticklabels()):
