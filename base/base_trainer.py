...
class BaseTrainer:
    ...
    def train(self):
        ...
        for metric in self.keys:
            ...
            metric_test_list = self.log.get('test_' + metric)
            if metric_test_list:
                plot_metric_all(np.array(metric_list), np.array(metric_val_list), np.array(metric_test_list), metric=metric, save_path=str(self.log_dir))
            else:
